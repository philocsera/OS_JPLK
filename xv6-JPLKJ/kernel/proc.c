...
found:
  ...
  // memory subsystem defaults (swap + quota)
  p->trace_mask = 0;
  p->mem_quota = 0;
...
growproc(int n)
{
  ...
  if(n > 0){
    // Growth path: quota check + uvmalloc + p->sz update all under p->lock.
    // swapout() of this proc (from another context) runs under the same
    // p->lock, so swapout_count is read consistently here. uvmalloc/walk/
    // kalloc never sleep -> holding a spinlock is safe; lock order is only
    // p->lock -> kmem.lock, with no reverse path -> no deadlock.
    acquire(&p->lock);
    sz = p->sz;
    ...
    // Quota is checked against RESIDENT pages (physical RAM in use). Pages
    // pushed out to swap don't occupy RAM, so they're excluded:
    //   outstanding = swapout_count - swapin_count  (net swapped pages)
    //   resident    = sz - outstanding*PGSIZE       (current RAM, bytes)
    // => swapping out frees quota headroom (quota+swap cooperate).
    // Backward compatible: no swap -> outstanding=0 -> resident=sz.
    if(p->mem_quota > 0){
      uint64 outstanding = (p->swapout_count > p->swapin_count)
                           ? (p->swapout_count - p->swapin_count) : 0;
      uint64 swapped = outstanding * PGSIZE;
      uint64 resident = (sz > swapped) ? (sz - swapped) : 0;   // clamp >= 0
      if(resident >= p->mem_quota || (uint64)n > p->mem_quota - resident){
        p->quota_denied_count++;
        release(&p->lock);            // print outside the lock
        printf("[quota denied] pid=%d name=%s sz=%d request=%d quota=%d resident=%d\n",
          p->pid, p->name, (int)sz, n, (int)p->mem_quota, (int)resident);
        return -1;
      }
    }
...
kfork(void)
{
  ...
  // uvmcopy's swap branch (swap_read/write_slot -> bread/bwrite) may sleep on
  // a buffer sleeplock. Sleeping while holding np->lock (a spinlock) would
  // panic ("sched locks"). np is still USED (not RUNNABLE), so dropping its
  // lock across the copy is safe — the scheduler won't pick it up.
  release(&np->lock);
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // Memory-quota inheritance: protected procs (init/sh) stay unlimited so the
  // system shell can never be quota-starved; everyone else inherits the
  // parent's quota. trace_mask follows the parent too.
...
procstat_get(int pid, struct procstat *out)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    if(p->pid == pid && p->state != UNUSED) {
      ...
      // Level 1: expose memory-pressure signals to the scheduling advisor.
      out->mem_quota = p->mem_quota;
      out->quota_denied_count = p->quota_denied_count;
      out->swapout_count = p->swapout_count;
      out->swapin_count = p->swapin_count;
...
procstat_all_range(int start, int end, struct procstat *dst, int dst_cap)
{
  ...
  for(int i = start; i < end && n < dst_cap; i++) {
    ...
    if(p->state != UNUSED) {
      ...
      // Level 1: expose memory-pressure signals to the scheduling advisor.
      dst[n].mem_quota = p->mem_quota;
      dst[n].quota_denied_count = p->quota_denied_count;
      dst[n].swapout_count = p->swapout_count;
      dst[n].swapin_count = p->swapin_count;
