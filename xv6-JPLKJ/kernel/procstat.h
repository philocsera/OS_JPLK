...
struct procstat {
  ...
  // ----- Level 1: shared observability of the memory subsystem -----
  // Read-only memory-pressure signals so the scheduling advisor can SEE that
  // a process is thrashing / quota-blocked. Filled from struct proc by
  // procstat_get / procstat_all_range. At Level 1 the advisor's decisions
  // stay scheduling-only — these are new INPUTS, not new control.
  uint64 mem_quota;          // bytes; 0 = unlimited
