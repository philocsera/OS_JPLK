...
def llm_classify(procs, model, host, timeout,
    ...
    sys_prompt = (
        ...
        "- swapout>5 (the process is THRASHING memory) -> prefer BATCH so it "
        "stops monopolizing the CPU while it pages; memory pressure outranks "
        "CPU class (Level 1: the scheduler now SEES memory state).\n"
    ...
    slim = [{"pid": p["pid"], "name": p["name"], "run": p["run"],
             "sleep": p["sleep"], "life": p["life"],
             "swapout": p.get("swapout", 0)} for p in procs]
