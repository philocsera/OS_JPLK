...
//
//   @@WL [{"pid":5,"name":"spin","run":40,"sleep":0,"ready":2,"life":42,"class":2,"group":5,"swapout":0,"swapin":0,"qdeny":0}, ...]
//
  ...
  for(;;) {
    ...
    for(int i = 0; i < n; i++) {
      ...
      if(!first)
        printf(",");
      first = 0;
      // Level 1: include memory-pressure signals (swapout/swapin/quota-denied)
      // so the host-side advisor prompt can SEE that a process is thrashing.
      printf("{\"pid\":%d,\"name\":\"%s\",\"run\":%ld,\"sleep\":%ld,"
             "\"ready\":%ld,\"life\":%ld,\"class\":%d,\"group\":%d,"
             "\"swapout\":%ld,\"swapin\":%ld,\"qdeny\":%ld}",
             p->pid, p->name, p->run_ticks, p->sleep_ticks,
             p->ready_ticks, p->lifetime, p->class_id, p->group_id,
             p->swapout_count, p->swapin_count, p->quota_denied_count);
    }
    printf("]\n");
